...
---@field table_object_char string which char used after select MARKDOWN TABLE operator to form a text-object
local config = {
...
M.setup = function(args)
  ...
  for _, mode in ipairs({ "x", "o" }) do
    vim.api.nvim_set_keymap(
      mode,
      "i" .. M.config.indent_object_char,
      ':<c-u>lua require("kontur").select_indent()<cr>',
      { noremap = true, silent = true }
    )
    vim.api.nvim_set_keymap(
      mode,
      "i" .. M.config.heading_object_char,
      ':<c-u>lua require("kontur").select_under_heading()<cr>',
      { noremap = true, silent = true }
    )
    vim.api.nvim_set_keymap(
      mode,
      "a" .. M.config.heading_object_char,
      ':<c-u>lua require("kontur").select_around_heading()<cr>',
      { noremap = true, silent = true }
    )
    vim.api.nvim_set_keymap(
      mode,
      "i" .. M.config.prefix_object_char,
      ':<c-u>lua require("kontur").select_prefix_block()<cr>',
      { noremap = true, silent = true }
    )
    vim.api.nvim_set_keymap(
      mode,
      "i" .. M.config.table_object_char,
      ':<c-u>lua require("kontur").select_markdown_table()<cr>',
      { noremap = true, silent = true }
    )
  end
end
...
M.select_markdown_table = function()
  return module.select_markdown_table()
end
